...
class PypoLiquidsoap:
    ...
    def play(self, media_item):
        if media_item["type"] == EventType.FILE:
            self.handle_file_type(media_item)
        elif media_item["type"] == EventType.EVENT:
            self.handle_event_type(media_item)
        elif media_item["type"] == EventType.STREAM_BUFFER_START:
            self.telnet_liquidsoap.start_web_stream_buffer(media_item)
        elif media_item["type"] == EventType.STREAM_OUTPUT_START:
            if (
                media_item["row_id"]
        ...
        elif media_item["type"] == EventType.STREAM_BUFFER_END:
            self.telnet_liquidsoap.stop_web_stream_buffer()
        elif media_item["type"] == EventType.STREAM_OUTPUT_END:
    ...
    def verify_correct_present_media(self, scheduled_now):
        ...
        try:
            scheduled_now_files = [
                x for x in scheduled_now if x["type"] == EventType.FILE
            ]

            scheduled_now_webstream = [
                x for x in scheduled_now if x["type"] in (EventType.STREAM_OUTPUT_START)
            ]
    ...
    def is_file(self, media_item):
        return media_item["type"] == EventType.FILE
